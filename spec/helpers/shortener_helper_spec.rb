...
describe Shortener::ShortenerHelper, type: :helper do
  describe '#short_url' do
    ...
    context 'without user or custom key' do
      before do
        expect(Shortener::ShortenedUrl).to receive(:generate).with(destination, owner: nil, expires_at: nil, fresh: false, category: nil).and_return(shortened_url)
      end
    ...
    context 'with owner' do
      ...
      it 'sends user to generate function' do
        expect(Shortener::ShortenedUrl).to receive(:generate).with(destination, owner: owner, expires_at: nil, fresh: false, category: nil)
    ...
    context 'with expires_at' do
      it 'sends custom key code to generate function' do
        expect(Shortener::ShortenedUrl).to receive(:generate).with(destination, owner: nil, expires_at: 'testtime', fresh: false, category: nil)
    ...
    context 'with category' do
      it 'sends custom key code to generate function' do
        expect(Shortener::ShortenedUrl).to receive(:generate).with(destination, owner: nil, expires_at: nil, fresh: false, category: 'testcat')
